...
from tkinter import Tk, Canvas, Widget, messagebox, font
...
# Lives Display
live_remaining = 3
live_text = c.create_text(canvas_width-10,10,anchor='ne', font=('Arial',10,'bold'), fill='darkblue', text='Lives : ' + str(live_remaining))
...
def lose_a_life():
    global live_remaining
    live_remaining -= 1
    c.itemconfigure(live_text, text='Lives: ' + str(live_remaining))

def catch_check():
    (catcher_x,catcher_y,catcher_x2,catcher_y2) = c.coords(catcher)
    for egg in eggs:
        (egg_x,egg_y,egg_x2,egg_y2) = c.coords(egg)
        if catcher_x < egg_x and egg_x2 < catcher_start_x2 and catcher_x2 - egg_y2 < 40:
            eggs.remove(egg)
            c.delete(egg)
